...
from rlgym_sim.utils.common_values import ORANGE_TEAM, BOOST_LOCATIONS
...
class AstraObs(AdvancedObs):
    ...
    def __init__(self, team_size=3, tick_skip: int = 8):
        ...
        self.boosts_timers = np.zeros(len(BOOST_LOCATIONS))
        self.boosts_state = np.zeros(len(BOOST_LOCATIONS))
        self.boosts_location = np.array(BOOST_LOCATIONS)
...
class MyObs(ObsBuilder):
    ...
    def build_obs(self, player: PlayerData, state: GameState, previous_action: np.ndarray) -> Any:
        ...
        for p in state.players:
            obs.extend(p.car_data.position)
